...
def main():
    ...
    docid_availability = load_availability_data('progress.json', 'doc.csv')
    
    # Process each bucket
    import glob
    bucket_files = glob.glob("user_bucket_*.csv")
    ...
    for file in bucket_files:
        ...
        if not train_df.empty and not test_df.empty:
            ...
            # Save results
            train_df.to_csv(f"train_{bucket_label}.csv", index=False)
            test_df.to_csv(f"test_{bucket_label}.csv", index=False)
