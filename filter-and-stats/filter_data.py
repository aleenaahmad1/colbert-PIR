...
def load_valid_urls(filename='backup.json'):
    try:
...
chunk_size = 10000
output_file = r'updated_buckets\filtered_data.csv'
first_chunk = True
valid_urls = load_valid_urls(r'updated_buckets\progress.json')  # Use a set instead of dict
...
for chunk in pd.read_csv(r'data\data.csv', delimiter='\t', chunksize=chunk_size):
    ...
    chunk.to_csv(output_file, mode='w' if first_chunk else 'a', header=first_chunk, index=False, delimiter='\t')
    first_chunk = False
    count+=1
